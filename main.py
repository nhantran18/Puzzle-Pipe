...
FILENAMES = [
    "2x1.json", "2x2.json", "3x3.json", "4x4.json", "5x5.json", "7x7.json", "10x10.json","15x15.json","20x20.json", "25x25.json"
]
GRAPHS = [
    ...
    pygame.font.SysFont('Corbel', 20) .render('15X15' , True , (0, 0, 0)),
    pygame.font.SysFont('Corbel', 20) .render('20x20' , True , (0, 0, 0)),
    pygame.font.SysFont('Corbel', 20) .render('25X25' , True , (0, 0, 0))
]
...
while True:
    CELL_WIDTH = 100 if mainGraph.row < 10 else 25
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT: 
            ...
            if width - 300 <= mouse[0] <= width - 100 and 5 <= mouse[1] <= 45:
                ...
                blind_mem_storage = tracemalloc.get_traced_memory()
            ...
            if width - 300 <= mouse[0] <= width - 100 and 50 <= mouse[1] <= 90:
                ...
                heuristic_mem_storage = tracemalloc.get_traced_memory()
                tracemalloc.stop()
                heuristic_end_time = time.time()
                # print(heuristic_mem_storage)
                drawDemoSolve(mainGraph, transforms, BASE_X_GRAPH, BASE_Y_GRAPH)
            ...
            if 250 <= mouse[0] <= 350 and 55 <= mouse[1] <= 95:
                mainGraph = readGraph(FILENAMES[7])

            if 375 <= mouse[0] <= 475 and 55 <= mouse[1] <= 95:
                mainGraph = readGraph(FILENAMES[8])
            
            if 500 <= mouse[0] <= 600 and 55 <= mouse[1] <= 95:
                mainGraph = readGraph(FILENAMES[9])
    ...
    HERISTIC_DURATION_TIME = pygame.font.SysFont('Corbel', 30).render(f"Heuristic:      {round(heuristic_end_time - heuristic_start_time, 4)} s                             {heuristic_mem_storage[1] - heuristic_mem_storage[0]} B" , True , (0, 0, 0))
    pygame.draw.rect(screen, (170, 170, 170), [0, height - 100, 900, 40]) 
    screen.blit(HERISTIC_DURATION_TIME , (20, height - 90))
    
    BLIND_DURATION_TIME = pygame.font.SysFont('Corbel', 30).render(f"Blind:             {round(blind_end_time - blind_start_time, 4)} s                             {blind_mem_storage[1] - blind_mem_storage[0]} B" , True , (0, 0, 0))
